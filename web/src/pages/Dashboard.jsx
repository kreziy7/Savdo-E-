...
export default function Dashboard() {
  ...
  function StatCard({ label, value, icon: Icon, color }) {
    const colorMap = {
      ...
      green: 'bg-[#EAF3E5] text-[#2D8B35]',
      amber: 'bg-amber-50 text-amber-600',
    };
    return (
      <div className="bg-white rounded-2xl border border-[#C6DEC0] p-5 flex flex-col gap-3">
        <div className="flex items-center justify-between">
          <span className="text-sm font-medium text-[#3C6B42]">{label}</span>
          <div className={`p-2 rounded-xl ${colorMap[color] || colorMap.green}`}>
            <Icon size={18} />
          </div>
        </div>
        <p className="text-2xl font-extrabold text-[#182A1A] leading-tight">{value}</p>
      </div>
    );
  }
  ...
  function SkeletonCard() {
    return (
      <div className="bg-white rounded-2xl border border-[#C6DEC0] p-5 flex flex-col gap-3 animate-pulse">
        <div className="flex justify-between">
          <div className="h-4 w-28 bg-[#C6DEC0] rounded" />
          <div className="h-8 w-8 bg-[#C6DEC0] rounded-xl" />
        </div>
        <div className="h-8 w-36 bg-[#C6DEC0] rounded" />
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-[#F5F8F3]">
      {/* Header */}
      <header className="bg-white border-b border-[#C6DEC0] px-5 py-4 flex items-center justify-between sticky top-0 z-20">
        <div className="flex items-center gap-3">
          <span className="text-xl font-extrabold text-[#2D8B35]">SAVDO</span>
          <span className="hidden sm:block text-sm text-[#3C6B42]">{todayDate}</span>
        </div>
        <div className="flex items-center gap-3">
          <span className="text-sm font-semibold text-[#182A1A]">{user?.name}</span>
          <button
      ...
      <div className="px-4 sm:px-6 py-6 max-w-3xl mx-auto flex flex-col gap-6">
        {/* Today's date mobile */}
        <p className="sm:hidden text-sm text-[#3C6B42] capitalize">{todayDate}</p>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
          {summaryLoading ? (
            ...
            <div className="col-span-3 bg-white rounded-2xl border border-red-100 p-5 flex flex-col items-center gap-2 text-center">
              ...
              <button onClick={() => refetchSummary()} className="text-sm text-[#2D8B35] font-semibold hover:underline">{t('retry')}</button>
            </div>
          ) : (
            <>
        ...
        <Link
          to="/sales"
          className="flex items-center justify-center gap-3 w-full bg-[#2D8B35] hover:bg-[#1D5E24] active:bg-[#1D5E24] text-white text-xl font-bold rounded-2xl transition-all shadow-md"
        ...
        {!summaryLoading && lowStock.length > 0 && (
          <div className="bg-white rounded-2xl border border-[#C6DEC0] overflow-hidden">
            <div className="flex items-center gap-2 px-5 py-4 border-b border-[#C6DEC0] bg-red-50">
              <AlertTriangle size={18} className="text-red-500" />
              <h3 className="font-bold text-[#182A1A]">{t('low_stock')}</h3>
            </div>
            <div className="divide-y divide-[#C6DEC0]">
              {lowStock.map((p) => (
                <div key={p._id} className="flex items-center justify-between px-5 py-3">
                  <span className="text-sm font-medium text-[#182A1A]">{p.name}</span>
        ...
        {/* Recent sales */}
        <div className="bg-white rounded-2xl border border-[#C6DEC0] overflow-hidden">
          <div className="flex items-center justify-between px-5 py-4 border-b border-[#C6DEC0]">
            <h3 className="font-bold text-[#182A1A]">{t('recent_sales')}</h3>
            <Link to="/sales" className="text-sm text-[#2D8B35] font-semibold hover:underline">
              {t('view_all')}
            </Link>
          </div>
          ...
          {salesLoading ? (
              ...
              {[1, 2, 3].map((i) => (
                <div key={i} className="flex items-center justify-between px-5 py-4 animate-pulse">
                  <div className="flex flex-col gap-2">
                    <div className="h-4 w-32 bg-[#C6DEC0] rounded" />
                    <div className="h-3 w-20 bg-[#EAF3E5] rounded" />
                  </div>
                  <div className="h-5 w-20 bg-[#C6DEC0] rounded" />
                </div>
              ))}
            </div>
          ) : salesError ? (
            <div className="px-5 py-10 text-center">
              ...
              <p className="text-sm font-semibold text-red-500">{t('error_loading')}</p>
              <button onClick={() => refetchSales()} className="mt-2 text-sm text-[#2D8B35] font-semibold hover:underline">{t('retry')}</button>
            </div>
          ) : recentSales.length === 0 ? (
            <div className="px-5 py-10 text-center text-[#3C6B42]">
              ...
              {recentSales.map((sale) => {
                ...
                return (
                  <div key={sale._id} className="flex items-center justify-between px-5 py-4">
                    <div>
                      <p className="text-sm font-semibold text-[#182A1A]">
                        {sale.productName || 'Mahsulot'}
                      </p>
                      <p className="text-xs text-[#3C6B42] mt-0.5">
                        {sale.quantity} {t(`unit_${sale.unit || 'pcs'}`)} · {timeStr}
                      </p>
                    </div>
                    <div className="text-right">
                      <p className="text-sm font-bold text-[#2D8B35]">
                        +{Number(sale.profit || 0).toLocaleString(locale)} {t('currency')}
                      </p>
                      <p className="text-xs text-[#3C6B42]">
